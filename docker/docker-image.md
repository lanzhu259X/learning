...
在 Docker Hub 上有大量的高质量的镜像可以用，这里我们就说一下怎么获取这些镜像。
...
想要列出已经下载下来的镜像，可以使用 `docker image ls` 命令。
...
我们可以用镜像的完整 ID，也称为 `长 ID`，来删除镜像。使用脚本的时候可能会用长 ID，但是人工输入就太累了，所以更多的时候是用 `短 ID` 来删除镜像。`docker image ls` 默认列出的就已经是`短 ID` 了，一般取前 3 个字符以上，只要足够区分于别的镜像就可以了。
...
要知道，当我们运行一个容器的时候（如果不使用卷的话），我们做的任何文件修改都会被记录于容器存储层里。而 `Docker` 提供了一个 `docker commit` 命令，可以将容器的存储层保存下来成为镜像。换句话说，就是 **在原有镜像的基础上，再叠加上容器的存储层，并构成新的镜像**。以后我们运行这个新镜像的时候，就会 **拥有原有容器最后的文件变化**。
...
这个指定很简单，就是设置环境变量而已，无论是后面的其它指令，如 RUN, 还是运行时的应用，都可以直接使用这里定义的环境变量。
...
这个例子中演示了任何环境，以及对含有空格的值使用双引号括起来的办法，这和 Shell 下的行为是一致的。
...
在这里先定义了环境变量 NODE_VERSION， 其后的 RUN 这层里，多次使用$NODE_VERSION 来进行操作制定。可以看到，将来升级镜像构建版本的时候，只需要更新 7.2.0 即可，Dockerfile 构建维护变得更轻松了。

下列制定可以支持环境变量展开：

ADD、COPY、ENV、EXPOSE、LABEL、USER、WORKDIR、VOLUME、STOPSIGNAL、ONBUILD。

可以从这个指令列表里感觉到，环境变量可以使用的地方很多，很强大。通过环境变量，我们可以让一份 Dockerfile 制作更多的镜像，只需要使用不同的环境变量即可。

### ARG 构建参数

格式：`ARG <参数名>[=<默认值>]`

构建蚕食和 ENV 的效果一样，都是设置环境变量。所不同的是，ARG 设置的构建环境的环境变量，在将来容器运行时是不会存在这些环境变量的。但是不要因此就是用 ARG 保存密码之类的信息，因为 Docker history 还是可以看到所有值的。
...
之前我们说过，容器运行时应该尽量保持容器存储层不发生写操作，对于数据库类需要保存动态数据的应用，其数据库文件应该保存于卷(volume)中，为了防止运行时用户忘记动态文件所保存默认挂载为卷，在 Dockerfile 中，我们可以事先指定某些目录挂载为匿名卷，这样在运行时如果用户不指定挂载，其应用也可以正常运行，不会向容器存储层写入大量数据。

```
VOLUME /data
```

这里的 `/data` 目录就会在运行时自动挂载为匿名卷，任何向 `/data` 中写入的信息都不会记录进容器存储成，从而保证了容器存储层的无状态。当然运行时可以覆盖这个挂载设置。不如：

```
docker run -d -v mydata:/data xxx
```

在执行这个命令中，就使用了 mydata 这个命令卷挂载到了 `/data` 这个位置，替代了 Dockerfile 中配置； Dockerfile 中定义的匿名卷的挂载配制。
